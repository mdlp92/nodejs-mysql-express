...
exports.create = (req, res) => {
  ...
  const meeting = new Meeting({date: req.body.date, time: req.body.time, content_link: req.body.content_link, content_title: req.body.content_title});
  ...
  Meeting.findById(req.params.meeting_ID, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({message: `Not found meeting with id ${req.params.meeting_ID}.`});
      } else {
        res.status(500).send({
...
exports.update = (req, res) => {
  ...
  Meeting.updateById(req.params.meeting_ID, new Meeting(req.body), (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({message: `Not found meeting with id ${req.params.meeting_ID}.`});
      } else {
...
exports.delete = (req, res) => {
  Meeting.remove(req.params.meeting_ID, (err, data) => {
    if (err) {
      ...
      res.send({message: `meeting was deleted successfully!`});
    }
  );
};
...
exports.deleteAll = (req, res) => {
  Meeting.removeAll((err, data) => {
    if (err) 
      res.status(500).send({
        message: err.message || "Some error occurred while removing all meetings."
      });
    else 
      res.send({message: `All meetings were deleted successfully!`});
    }
  );
};
