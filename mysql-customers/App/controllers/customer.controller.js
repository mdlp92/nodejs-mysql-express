  ...
  Customer.findById(req.params.customer_ID, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({message: `Not found Customer with id ${req.params.customer_ID}.`});
      } else {
        res.status(500).send({
...
exports.update = (req, res) => {
  ...
  Customer.updateById(req.params.customer_ID, new Customer(req.body), (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({message: `Not found Customer with id ${req.params.customer_ID}.`});
      } else {
